...
export default function PagosCobrosAdmin() {
  const [sub, setSub]       = useState('pagos') // 'pagos' | 'cobros'
  ...
  async function cargar() {
    ...
    try {
      ...
      const [pRes, coRes, prRes, gRes] = await Promise.all([
      ...
      const [p, co, pr, g] = await Promise.all([pRes.json(), coRes.json(), prRes.json(), gRes.json()])
